...
exports.getValue = function (div) {
    ...
    var inputValues = div.getAttribute('inputValues').split(',');
    return inputValues[0] + ':' + inputValues[1];

};

exports.setValue = function (div, hour, minute) {
    ...
    var textInput = getTextInput(div);
...
exports.setEnabled = function (div, setting) {
    ...
    var hourSpan, colonSpan, minuteSpan, textInput;

    div.setAttribute('enabled', setting ? 'true' : 'false');

    hourSpan = getMinuteSpan(div);
    colonSpan = getColonSpan(div);
    minuteSpan = getHourSpan(div);
...
function getInputValuesFromChild (node) {
    ...
    var inputValues = getAttributeValue(node, 'inputValues').split(',');
...
function highlightInput (node) {
    ...
    var index, hourSpan, minuteSpan, deselectedColor;

    index = getSelectedIndex(node);

    hourSpan = getHourSpanFromChild(node);
    minuteSpan = getMinuteSpanFromChild(node);

    deselectedColor = getTextInputFromChild(node).style.background;
...
function setHour (node, value) {
    ...
    var inputValues = getInputValuesFromChild(node);
...
function nextSelection (node) {
    ...
    var currentSelectedIndex, newSelectedIndex;

    currentSelectedIndex = getSelectedIndex(node);
    newSelectedIndex = (currentSelectedIndex + 1 >= 2) ? 1 : currentSelectedIndex + 1;
...
function previousSelection (node) {
    ...
    currentSelectedIndex = getSelectedIndex(node);
    newSelectedIndex = (currentSelectedIndex - 1 < 0) ? 0 : currentSelectedIndex - 1;
...
function getMaxValue (node) {
    ...
    var maxValue, selectedIndex, inputValues;
...
function updateHourValue (node, inputKey) {
    ...
    var newValue, maxValue, selectedIndex, inputValues;

    maxValue = getMaxValue(node);
    selectedIndex = getSelectedIndex(node);
...
function incrementValue (node) {
    ...
    var newValue, maxValue, inputValues, selectedIndex;
...
function handleKeyDown (e) {
    ...
    var selectedIndex, patt, numbersEntered;
...
class TimeInput extends HTMLElement {
    ...
    connectedCallback () {
        ...
        hourSpanNode = document.createElement('span');
        hourSpanNode.className = 'hour-span';
        hourSpanNode.innerText = '00';

        colonSpanNode = document.createElement('span');
        colonSpanNode.className = 'colon-span';
        colonSpanNode.innerText = ':';

        minuteSpanNode = document.createElement('span');
